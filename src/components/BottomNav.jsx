...
export default function BottomNav(props) {
  ...
  const fabXPosition = useMemo(() => {
    switch (fabPosition) {
      case 'right':
        return sizing.width - padding - fabRadius - cutoutWidth;
      case 'left':
        return padding + fabRadius + cutoutWidth;
      case 'center':
      ...
      <defs>
        <mask id="fab-slot">
          ...
          <AnimatePresence>
            <motion.circle
              initial={{ r: 0 }}
              animate={{ r: 40, y: scrollDelta > 0 ? -65 : 0 }}
              r="0"
              cx={fabXPosition}
              cy="47"
              fill="black"
      ...
      <g transform={`translate(${fabXPosition - fabRadius} 14)`}>
        <AnimatePresence>
          <motion.g
            ...
            animate={{
              scale: 1,
              y: scrollDelta > 0 ? 79 - padding - cutoutWidth - fabRadius : 0,
            }}
            exit={{ scale: 0 }}
            whileTap={{ scale: 0.8 }}
            onClick={onFabClick}
