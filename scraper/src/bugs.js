...
module.exports = async function getBugs(browser) {
  ...
  console.log(`queueing ${bugs.length} bugs`);
  let results = [];

  while (bugs.length) {
    let nextChunk = await Promise.all(
      bugs.splice(0, pageCount).map(bug => getBugData(browser, bug))
    );
...
async function getBugData(browser, bug) {
  const page = await browser.newPage();
  await page.goto(bug.href, { waitUntil: 'networkidle2' });

  page.on('pageerror', err => {
    console.log('Error', bug.title, err.toString());
  });
